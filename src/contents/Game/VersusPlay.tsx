...
const reversi = new Reversi('VERSUS', [
...
const VersusPlay = ({ playerStone }: Props): JSX.Element => {
    ...
    if (!gameInfo.isGameEnd) {
        if (!((reversi.isBlackTurn && playerStone === 'BLACK') || (!reversi.isBlackTurn && playerStone === 'WHITE'))) {
            canClick = false;

            setTimeout(() => {
                const [row, column] = reversi.searchBestCoords();
                ...
                setReversiBoard(structuredClone(reversi.board));
            }, 750);
        }
    }
